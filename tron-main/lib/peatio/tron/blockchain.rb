    ...
    class Blockchain < Peatio::Blockchain::Abstract
      ...
      def load_balance_of_address!(address, currency_id)
        ...
        if currency.dig(:options, :trc10_token_id).present?
          client.json_rpc(path: 'wallet/getaccount',
                          params: { address: refactor_decode_address(address) }
          ).fetch('assetV2', [])
          ...
          client.json_rpc(path: 'wallet/triggersmartcontract',
                          params: {
                            owner_address: refactor_decode_address(address),
                            contract_address: refactor_decode_address(currency.dig(:options, :trc20_contract_address)),
                            function_selector: 'balanceOf(address)',
                            parameter: abi_encode(refactor_decode_address(address)[2..42]) }
      ...
      def build_trc10_transaction(tx_hash)
        ...
        currencies.each do |currency|
          formatted_txs << { hash: refactor_txid(tx_hash['txID']),
                             amount: convert_from_base_unit(tx['parameter']['value']['amount'], currency),
                             to_address: refactor_encode_address(tx['parameter']['value']['to_address']),
                             txout: 0,
      ...
      def build_trc20_transaction(tx_hash)
        ...
        tx_hash.fetch('log', []).each_with_index do |log, index|
          ...
          currencies = @trc20.select do |c|
            c.dig(:options, :trc20_contract_address) == refactor_encode_address("41#{log.fetch('address')}")
          end
          next if currencies.blank?

          destination_address = refactor_encode_address("41#{log.fetch('topics').last[-40..-1]}")

          currencies.each do |currency|
            formatted_txs << { hash: refactor_txid(tx_hash.fetch('id')),
      ...
      def build_coin_transaction(tx_hash)
        tx = tx_hash['raw_data']['contract'][0]
        @trx.map do |currency|
          { hash: refactor_txid(tx_hash['txID']),
            amount: convert_from_base_unit(tx['parameter']['value']['amount'], currency),
            to_address: refactor_encode_address(tx['parameter']['value']['to_address']),
            txout: 0,
            block_number: tx_hash['block_number'],
      ...
      def build_invalid_trc20_transaction(tx_hash)
        currencies = @trc20.select do |c|
          c.dig(:options, :trc20_contract_address) == refactor_encode_address(tx_hash.fetch('contract_address'))
        end
        return [] if currencies.blank?

        currencies.each_with_object([]) do |currency, invalid_txs|
          invalid_txs << { hash: refactor_txid(tx_hash.fetch('txID')),
